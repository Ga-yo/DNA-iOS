...
class CViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        guard let commentVC = storyboard?.instantiateViewController(identifier: "CCommentVC") as? CCommentViewController else {return}
    ...
    private func getList() {
        HTTPClient().get(url: ListAPI.timeLine("COMMON").path(), params: nil, header: Header.token.header()).responseJSON(completionHandler: {[unowned self] res in
